...
この修正はまだコミットしていません（ユーザーの明示的な指示があってからコミットする方針のため）。
...
### 9. 本番へコードを反映
現状 `main` ブランチにはこの分析機能のコードがまだ入っていません（`wip/analytics-fix` にのみ存在）。環境変数を登録しただけでは本番には反映されません。以下のいずれかが必要です。
- このブランチを `main` にマージしてから本番デプロイする（PRを作る場合は私が用意できるので、必要なら声をかけてください）。
- あるいはVercel側でこのブランチをProductionにプロモートする運用にする。

どちらの方法にするか、次にお伝えいただければ実施します。
...
- `Code.gs` の2件のフィールド名修正をコミットしてよいか（このHANDOFF作成時点では未コミット）。
- `wip/analytics-fix` を `main` にマージする方法（PR作成 or 直接マージ or Vercel側でブランチ切り替え）。
