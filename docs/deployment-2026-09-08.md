...
アクセス解析の`/api/analytics/events`では`analytics_webhook_rejected`による502を確認した。修正前のデプロイ`dpl_3pMr3Z85XfKMU8oJwDzTqXWnhpca`でも直近24時間の取得上限50件すべてに同じエラーがあり、今回の反映前から発生している。分析GASの版6へリクエストが届くことは確認できたが、Cloudログは利用できず、共有秘密・イベント検証・書込ロックなどのどの条件で拒否されたかは未特定。解析連携は正常確認済みとは扱わない。
